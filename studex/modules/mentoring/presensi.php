...
//  modules/mentoring/presensi.php — Input Presensi Mentoring
// ============================================================
...
// Validasi sesi
$stmt = $db->prepare("
    SELECT m.*, a.nama AS nama_angkatan, a.id AS angkatan_id_val
...
$siswas = $db->prepare("
    ...
    ORDER BY nama ASC
");
$siswas->execute([$sesi['angkatan_id']]);
...
$existingStmt = $db->prepare("
    ...
    FROM presensi
    WHERE referensi_id = ? AND modul = 'mentoring'
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $db->prepare("
            DELETE FROM presensi WHERE referensi_id = ? AND modul = 'mentoring'
        ")->execute([$id]);
        ...
        foreach ($siswas as $s) {
            $status = sanitize($presensiData[$s['id']] ?? 'hadir');
        ...
        redirect(url('modules/mentoring/presensi.php?id=' . $id));
    }
}
    ...
    <div class="card">
        <div class="empty-state">
            ...
            <div class="empty-state-desc">
                Belum ada siswa aktif di angkatan <?= e($sesi['nama_angkatan']) ?>.
...
<form method="POST" action="?id=<?= $id ?>" id="presensiForm">
    ...
    <div class="card">
                    ...
                    <tr class="presensi-row" data-status="<?= $curStatus ?>">
                        ...
                        <td>
                            <div class="status-group">
                                <?php foreach ([
                                    'hadir' => 'btn-status-hadir',
                                    'izin'  => 'btn-status-izin',
                                    'sakit' => 'btn-status-sakit',
                                    'alpha' => 'btn-status-alpha',
                                ] as $val => $cls): ?>
                                    <label class="status-btn <?= $cls ?> <?= $curStatus === $val ? 'active' : '' ?>">
                                        <input type="radio"
                                               name="presensi[<?= $s['id'] ?>]"
                                               value="<?= $val ?>"
        ...
        <div class="flex items-center justify-between"
             style="padding:20px 24px; border-top:1px solid var(--border);">
