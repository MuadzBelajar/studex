...
//  modules/rabuan/create.php — Tambah Rapat Rabuan
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$input['judul'])
        $errors['judul'] = 'Judul rapat wajib diisi.';
    if (!$input['tanggal'])
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("
            INSERT INTO rabuan
                (angkatan_id, judul, tanggal, waktu_mulai, waktu_selesai,
                 lokasi, agenda, status, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $input['angkatan_id'],
        ...
        setFlash('success', 'Rapat "' . $input['judul'] . '" berhasil ditambahkan.');
        redirect(url('modules/rabuan/detail.php?id=' . $newId));
    }
}
...
<div class="card" style="max-width:760px;">
    <div class="card-header">
        <div class="card-title">Form Tambah Rapat Rabuan</div>
    </div>
    ...
    <form method="POST" action="" novalidate>
        ...
        <div class="form-group mb-5">
            ...
            <input type="text" name="judul"
                   ...
                   value="<?= e($input['judul'] ?? '') ?>"
                   placeholder="Contoh: Rabuan Pekan 3 — Evaluasi Program">
        ...
        <div class="form-row mb-5">
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-control">
                    <option value="terjadwal"   <?= ($input['status'] ?? 'terjadwal') === 'terjadwal'   ? 'selected' : '' ?>>Terjadwal</option>
                    <option value="berlangsung" <?= ($input['status'] ?? '') === 'berlangsung' ? 'selected' : '' ?>>Berlangsung</option>
                    <option value="selesai"     <?= ($input['status'] ?? '') === 'selesai'     ? 'selected' : '' ?>>Selesai</option>
                    <option value="dibatalkan"  <?= ($input['status'] ?? '') === 'dibatalkan'  ? 'selected' : '' ?>>Dibatalkan</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group mb-5">
            ...
            <input type="text" name="lokasi" class="form-control"
                   ...
                   placeholder="Ruang rapat, aula, dll.">
        </div>
        ...
        <div class="form-group mb-6">
            ...
            <textarea name="agenda" class="form-control" rows="4"
                      placeholder="Tulis agenda atau deskripsi rapat..."><?= e($input['agenda'] ?? '') ?></textarea>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="<?= url('modules/rabuan/index.php') ?>" class="btn btn-secondary">Batal</a>
        </div>
